...
export const useSelectClasses = (props: SelectProps) => {
  return {
    container: () =>
      classNames(
        'jds-select',
        `jds-select__container--color--${props.color}`,
        props.className
      ),
    control: (
      state: ControlProps<Selectable, boolean, GroupBase<Selectable>>
    ) => {
      return classNames(
        'jds-select__control',
        `jds-select__control--${props.appearance}${
          state.menuIsOpen || state.isFocused ? '--active' : ''
        }`
      );
    },
    valueContainer: () => 'jds-select-value-container',
    placeholder: (
      state: PlaceholderProps<Selectable, boolean, GroupBase<Selectable>>
    ) =>
      classNames(
        'jds-select-placeholder',
        `jds-select-placeholder--position--${props.labelPosition}${
          props.isMulti ? '--multi' : ''
        }`,
        {
          'jds-select-placeholder--transformed':
