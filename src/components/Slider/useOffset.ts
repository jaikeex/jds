...
export const useOffset = (ref: React.RefObject<HTMLInputElement>) => {
  ...
  const min =
    ref.current && ref.current.hasAttribute('min')
      ? Number(ref.current.getAttribute('min'))
      : 0;

  const max =
    ref.current && ref.current.hasAttribute('max')
      ? Number(ref.current.getAttribute('max'))
      : 100;
